...
function processJsonFile(filePath) {
  try {
    ...
    // For location.json, handle nested structure with location keys
    if (fileName === 'location') {
      ...
      // Extract all imagekit URLs from locations object
      if (jsonData.locations && typeof jsonData.locations === 'object') {
        Object.entries(jsonData.locations).forEach(([locationKey, locationData]) => {
          if (typeof locationData === 'object' && locationData !== null) {
            const locationImagekitKeys = findImagekitKeys(locationData, false);
            if (Object.keys(locationImagekitKeys).length > 0) {
              result.locations[locationKey] = locationImagekitKeys;
            }
          }
        });
      }
      
      // If no bannerImage URL and no locations found, return empty structure
      if (!result.bannerImage && Object.keys(result.locations).length === 0) {
        return {};
      }
    ...
    // For blogsCategoryMetas, handle nested structure with category keys
    if (fileName === 'blogsCategoryMetas') {
      const result = {
        categories: {}
      };
      
      // Extract all imagekit URLs from categories object
      if (typeof jsonData === 'object' && !Array.isArray(jsonData)) {
        Object.entries(jsonData).forEach(([categoryKey, categoryData]) => {
          if (typeof categoryData === 'object' && categoryData !== null) {
            const categoryImagekitKeys = findImagekitKeys(categoryData, false);
            if (Object.keys(categoryImagekitKeys).length > 0) {
              result.categories[categoryKey] = categoryImagekitKeys;
            }
          }
        });
      }
